...
class HashTable():
    def __init__(self, grid, dims):
        ...
        self.gtable = grid
        self.htable = torch.full(self.shape, fill_value=-1, dtype=grid.dtype, device=grid.device)
        self.dims = dims
        self.D = len(dims)
        self.iter = 0
    ...
    def sort(self):
        ...
        sorted_vals, _ = torch.sort(self.htable[self.dslice[it]], dim=d)
        self.htable[self.dslice[it]] = sorted_vals

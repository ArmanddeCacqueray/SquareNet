...
class HashTable():
    def __init__(self, grid, dims):
        ...
        self.gtable = grid
        self.htable = jnp.full(self.shape, fill_value=-1, dtype=grid.dtype)
        self.dims = dims
        self.D = len(dims)
        self.iter = 0
    ...
    def sort(self):
        ...
        for hsl, gsl in zip(self.hslice[it], self.gslice[it]):
            self.htable = self.htable.at[hsl].set(self.gtable[gsl])

        self.htable = self.htable.at[self.dslice[it]].set(
            jnp.sort(self.htable[self.dslice[it]], axis=d)
        )

        for hsl, gsl in zip(self.hslice[it], self.gslice[it]):
            self.gtable = self.gtable.at[gsl].set(self.htable[hsl])
